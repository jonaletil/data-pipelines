...
class StageToRedshiftOperator(BaseOperator):
    """
    Class for staging JSON data from S3 to Redshift

    Attributes:
        redshift_conn_id: redshift connection
        aws_credentials: aws credentials / iam user
    ...
    def execute(self, context):
        ...
        if self.execution_date:
            formatted_sql = StageToRedshiftOperator.copy_sql_by_date.format(
                self.table,
                s3_path,
        ...
        self.log.info(f"Copying completed for table: {self.table}")
